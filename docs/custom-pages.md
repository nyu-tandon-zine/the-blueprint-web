...
- It's displayed inside a **sandboxed frame**, so your code is completely isolated — you can't accidentally break the rest of the site, and nothing you write can affect other works. Design boldly.
- The site hands your page the work's real content (title, author, text, images) so you don't hard-code it.
- Your page tells the site how tall it is, so it fits seamlessly with no inner scrollbar.
- Every work keeps its default page. If your custom page isn't finished, the work still ships — it just shows the default. **You can never block the issue.**

---

## Step by step

### 1. Find your work's ID
...
This shows your page inside the actual frame, with real content and the height-fitting behaviour — exactly how a reader will see it. You do **not** need to register your page to preview it.
...
1. Add your work ID to `src/custom-pages/registry.ts`:
   ...
   export const CUSTOM_PAGES: Record<string, CustomPageConfig> = {
     '3f9c1a2b-...': {},                     // reader sees default first, can toggle to custom
     // '3f9c1a2b-...': { defaultView: 'custom' },  // OR: custom shows first
   }
   ```
2. Open a pull request (see the checklist at the bottom). A web lead reviews it on the preview deploy and merges.

---

## The content contract

When the page loads, the site sends your work's content. Define a function called `onWorkData` and it will be called with this object:
...
These are enforced by the site's security policy. If you break one, that resource silently won't load — so know them up front:
...
- **Design mobile-first.** Readers are often on phones; test narrow widths.
...
- [ ] Reads content via onWorkData (title/text not hard-coded)
- [ ] Looks right on mobile (tested at ~375px wide)
- [ ] No console errors in the preview; height fits with no inner scrollbar
- [ ] Work ID added to src/custom-pages/registry.ts
- [ ] Boilerplate block left unchanged
```

Reviewers will also confirm the page stays inside the sandbox (no `allow-same-origin`, no network calls) before merging.
